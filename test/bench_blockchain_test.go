...
func MockTxsP2PKH(keyDirPath string, testDB dbm.DB, txNumber, otherAssetNum int) ([]*types.Tx, error) {
	...
	xpub, _, err := hsm.XCreate("TestP2PKH", "password")
	if err != nil {
		return nil, err
	}
...
func MockTxsP2SH(keyDirPath string, testDB dbm.DB, txNumber, otherAssetNum int) ([]*types.Tx, error) {
	...
	xpub1, _, err := hsm.XCreate("TestP2SH1", "password")
	if err != nil {
		return nil, err
	}

	xpub2, _, err := hsm.XCreate("TestP2SH2", "password")
	if err != nil {
		return nil, err
	}
...
func MockTxsMultiSign(keyDirPath string, testDB dbm.DB, txNumber, otherAssetNum int) ([]*types.Tx, error) {
	...
	xpub1, _, err := hsm.XCreate("TestMultilNodeSign1", "password1")
	if err != nil {
		return nil, err
	}

	xpub2, _, err := hsm.XCreate("TestMultilNodeSign2", "password2")
	if err != nil {
		return nil, err
	}
	txs := []*types.Tx{}
