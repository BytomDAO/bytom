...
func BuildContractTransaction(args []string, minArgsCount int, alias bool, btmGas string) (string, error) {
	...
	case "TradeOffer":
		...
		if selector == ClauseTrade {
			if len(args) != minArgsCount+5 {
				buf = fmt.Sprintf("the number of arguments[%d] for clause 'trade' in contract 'TradeOffer' is not equal to 5", len(args)-5)
				err = errors.New(buf)
				return emptyStr, err
			}
			...
			if len(args) != minArgsCount+4 {
				buf = fmt.Sprintf("the number of arguments[%d] for clause 'cancel' in contract 'TradeOffer' is not equal to 4", len(args)-5)
				err = errors.New(buf)
				return emptyStr, err
			}
	...
	case "Escrow":
		...
		if selector == ClauseApprove || selector == ClauseReject {
			if len(args) != minArgsCount+5 {
				buf = fmt.Sprintf("the number of arguments[%d] for clause 'approve' or 'reject' in contract 'Escrow' is not equal to 5", len(args)-5)
				err = errors.New(buf)
				return emptyStr, err
			}
	...
	case "LoanCollateral":
		...
		if selector == ClauseRepay {
			if len(args) != minArgsCount+6 {
				buf = fmt.Sprintf("the number of arguments[%d] for clause 'repay' in contract 'LoanCollateral' is not equal to 6", len(args)-5)
				err = errors.New(buf)
				return emptyStr, err
			}
			...
			if len(args) != minArgsCount+1 {
				buf = fmt.Sprintf("the number of arguments[%d] for clause 'default' in contract 'LoanCollateral' is not equal to 1", len(args)-5)
				err = errors.New(buf)
				return emptyStr, err
			}

			buf = fmt.Sprintf("the clause was not implemented")
			err = errors.New(buf)
			return emptyStr, err
		} else if selector == ClauseEnding {
			buf = fmt.Sprintf("no clause was selected in this program, ending exit")
	...
	case "CallOption":
		buf = fmt.Sprintf("the contract was not implemented")
		err = errors.New(buf)
		return emptyStr, err
...
func AddContractArguments(tpl *txbuilder.Template, contractName string, contractArgs []string) (*txbuilder.Template, error) {
	...
	case "Escrow":
		...
		tpl, errArgs = AddPubValueWitness(tpl, rootPub, path1, path2, selector)
	case "LoanCollateral":
		if !(len(contractArgs) == 1 || len(contractArgs) == 6) {
			buf = fmt.Sprintf("the arguments of contract 'LoanCollateral' is not equal to 1 or 6")
			err = errors.New(buf)
			return nil, err
		}
	...
	case "CallOption":
		if !(len(contractArgs) == 1 || len(contractArgs) == 8) {
			buf = fmt.Sprintf("the arguments of contract 'CallOption' is not equal to 1 or 8")
			err = errors.New(buf)
			return nil, err
		}
		// Not implemented
