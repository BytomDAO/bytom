...
func newBlockKeeper(chain *protocol.Chain, peers *node.PeerSet, quitReqBlockCh chan *string) *blockKeeper {
	bk := &blockKeeper{
		chain:            chain,
		peers:            peers,
...
func (bk *blockKeeper) BlockRequestWorker(peerID string, maxPeerHeight uint64) error {
	...
	for num <= maxPeerHeight && num > 0 {
		...
		if errors.Root(err) == errPeerDropped || errors.Root(err) == errGetBlockTimeout || errors.Root(err) == errReqBlock {
			...
			if ban := bk.peers.AddBanScore(peerID, 0, 50, "block request error"); !ban {
				bk.peers.InitiativeRemovePeer(peerID)
			}
			break
		}
		isOrphan, err = bk.chain.ProcessBlock(block)
		if err != nil {
			bk.peers.AddBanScore(peerID, 50, 0, "block process error")
...
func (bk *blockKeeper) txsProcessWorker() {
	for txsResponse := range bk.txsProcessCh {
		...
		if isOrphan, err := bk.chain.ValidateTx(tx); err != nil && isOrphan == false {
			bk.peers.AddBanScore(txsResponse.peerID, 50, 0, "tx error")
		}
	}
}
