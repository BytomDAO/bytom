...
func MakeSwitchPair(t testing.TB, testDB dbm.DB, initSwitch func(int, *Switch) *Switch) (*Switch, *Switch) {
	...
	switches := MakeConnectedSwitches(testCfg, 2, testDB, initSwitch, Connect2Switches)
...
func TestSwitchFiltersOutItself(t *testing.T) {
	...
	s1 := MakeSwitch(testCfg, 1, testDB, initSwitchFunc)
	s1.Start()
	defer s1.Stop()
...
func TestSwitchBannedPeer(t *testing.T) {
	dirPath, err := ioutil.TempDir(".", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSwitchDuplicatePeer(t *testing.T) {
	dirPath, err := ioutil.TempDir(".", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSwitchAddInboundPeer(t *testing.T) {
	dirPath, err := ioutil.TempDir(".", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	s1 := MakeSwitch(&cfg, 1, testDB, initSwitchFunc)
	s1.Start()
	defer s1.Stop()
...
func TestSwitchStopPeer(t *testing.T) {
	dirPath, err := ioutil.TempDir(".", "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSwitchUpdateNodeInfo(t *testing.T) {
	dirPath, err := ioutil.TempDir(".", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	bestHeight := uint64(1024)
	bestHash := bc.Hash{V0: 1}
	s1.UpdateNodeInfoHeight(bestHeight, bestHash)
	if s1.NodeInfo().BestHash != bestHash || s1.NodeInfo().BestHeight != bestHeight {
		t.Fatalf("TestSwitchUpdateNodeInfo want:%v %s got:%v %s", bestHeight, bestHash.String(), s1.NodeInfo().BestHeight, s1.NodeInfo().BestHash.String())
	}
}
