...
//defines the language types of mnemonic.
const (
	ChineseSimplified = iota
	ChineseTraditional
	English
	Italian
	Japanese
...
func init() {
	...
	wordList = [][]string{wordlists.ChineseSimplified,
...
func SetWordMap(language int) (map[string]int, error) {
	if language < 0 || language >= MaxSupportLanguage {
		return nil, ErrLanguageTypeIncorrect
	}
	wordMap := map[string]int{}
	for i, v := range wordList[language] {
		wordMap[v] = i
	}
	return wordMap, nil
}
...
// SetWordList sets the list of words to use for mnemonics. Currently the list
// that is set is used package-wide.
func SetWordList(language int) ([]string, error) {
	if language < 0 || language >= len(wordList) {
		return nil, ErrLanguageTypeIncorrect
	}
...
func EntropyFromMnemonic(mnemonic string, language int) ([]byte, error) {
...
func NewMnemonic(entropy []byte, language int) (string, error) {
...
func MnemonicToByteArray(mnemonic string, language int, raw ...bool) ([]byte, error) {
	var (
...
func NewSeedWithErrorChecking(mnemonic string, password string, language int) ([]byte, error) {
...
func IsMnemonicValid(mnemonic string, language int) bool {
