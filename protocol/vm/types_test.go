...
func TestBigIntBytes(t *testing.T) {
	cases := []struct {
		...
		{new(big.Int).SetInt64(1 << 32), []byte{0x34, 0x32, 0x39, 0x34, 0x39, 0x36, 0x37, 0x32, 0x39, 0x36}},
		{
			new(big.Int).Exp(new(big.Int).SetInt64(10), new(big.Int).SetInt64(32), nil),
			[]byte{0x31, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30,
	...
	for _, c := range cases {
		...
		if !bytes.Equal(gotData, c.data) {
			t.Errorf("BigIntBytes(%s) = %x want %x", c.num.String(), gotData, c.data)
		}
	}
}
...
func TestAsBigInt(t *testing.T) {
	cases := []struct {
		...
		{new(big.Int).SetInt64(1 << 32), []byte{0x34, 0x32, 0x39, 0x34, 0x39, 0x36, 0x37, 0x32, 0x39, 0x36}, nil},
		{
			new(big.Int).Exp(new(big.Int).SetInt64(10), new(big.Int).SetInt64(32), nil),
			[]byte{0x31, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30,
				0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30},
