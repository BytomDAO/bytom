...
func TestEstimateTxGasWithSingleSign(t *testing.T) {
	...
	realTotalNeu := float64(2095 * consensus.VMGasRate)
	rate := math.Abs((float64(estimateResult.TotalNeu) - realTotalNeu) / float64(estimateResult.TotalNeu))
	if rate > 0.2 {
...
func TestEstimateTxGasWithMultiSign_2_1(t *testing.T) {
	...
	realTotalNeu := float64(3305 * consensus.VMGasRate)
	rate := math.Abs((float64(estimateResult.TotalNeu) - realTotalNeu) / float64(estimateResult.TotalNeu))
	if rate > 0.2 {
		t.Errorf(`the estimateNeu over realNeu 20%%`)
	}
...
func TestEstimateTxGasWithMultiSign_5_3(t *testing.T) {
	...
	realTotalNeu := float64(13556 * consensus.VMGasRate)
	rate := math.Abs((float64(estimateResult.TotalNeu) - realTotalNeu) / float64(estimateResult.TotalNeu))
	if rate > 0.2 {
		t.Errorf(`the estimateNeu over realNeu 20%%`)
	}
